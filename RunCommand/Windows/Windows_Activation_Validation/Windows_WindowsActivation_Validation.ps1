...
Disclaimer:
    ...
    For more details, see: https://aka.ms/AzVmWinActivationValidationTool

.SYNOPSIS
    Validates Windows activation status on Azure VMs and provides troubleshooting guidance.

.DESCRIPTION
    This script performs the following checks:
    - Ensures WinRM service is running
    - For Azure Edition images, validates IMDS endpoint connectivity and certificate chain
    - Tests TCP connectivity to the KMS endpoint
    - Checks Windows activation status and attempts activation if needed
    - Provides relevant troubleshooting links for common activation error codes
...
.EXAMPLE
    ...
    PS> .\Windows_WindowsActivation_Validation.ps1
#>
