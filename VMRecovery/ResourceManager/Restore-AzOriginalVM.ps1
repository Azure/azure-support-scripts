...
$authContext = Set-AzContext -Subscription $subscriptionId
if (-not $authContext.Subscription.Id)
{
    $message = "[Error] Unable to set context to subscriptionId $subscriptionId. Run Connect-AZAccount and then run the script again."
...
$vm = Get-AzVM -ResourceGroupName $resourceGroupName -Name $vmName -WarningAction SilentlyContinue
if (-not $vm)
{
...
else
{
    ...
    $restoreOriginalStateScriptPath = (get-childitem $restoreOriginalStateFile).FullName
    $null = set-AzVMOSDisk -vm $vm -Name $diskName -ManagedDiskId $problemvmOsDiskManagedDiskID -WarningAction SilentlyContinue
    $null = Update-AzVM -ResourceGroupName $resourceGroupName -VM $vm 
    write-log "[Success] Swapped OS disk for problem VM $vmName" -color green
}
...
$started = Start-AzVM -ResourceGroupName $resourceGroupName -Name $vmName
if ($started)
{
...
if ($windowsVM -and -not (RanFromCloudShell))
{
    ...
    $null = Get-AzRemoteDesktopFile -ResourceGroupName $resourceGroupName -Name $vmName -Launch
}
...
if ((read-host) -eq 'Y')
{
    ...
    $result = Remove-AzResourceGroup -Name $rescueResourceGroupName -force
    if ($result)
    {
