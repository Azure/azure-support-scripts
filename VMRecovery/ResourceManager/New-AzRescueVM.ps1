...
.EXAMPLE
    ...
    $scriptResult = .\New-AzRescueVM.ps1 -resourceGroupName sujtemp -VmName sujnortheurope -subscriptionId xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx

.EXAMPLE
    Examples with optional parametersm in this example it will create the rescue VM with RedHat installed

    $scriptResult = .\New-AzRescueVM.ps1 -VmName ubuntu -resourceGroupName portalLin -subscriptionId xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx -Publisher RedHat -Offer RHEL -Sku 7.3 -Version 7.3.2017090723 -prefix rescuered 

.EXAMPLE
    $scriptResult = .\New-AzRescueVM.ps1 -resourceGroupName sujtemp -VmName sujnortheurope -subscriptionId xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx -UserName "sujasd" -Password "XPa55w0rrd12345" -prefix "rescuex2"

.EXAMPLE
    Example for managed disk VM:

    $scriptResult =  .\New-AzRescueVM.ps1 -resourceGroupName sujasrg -VmName sujunmanagedP -subscriptionId xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx -UserName "sujasd" -Password "XPa55w0rrd12345" -prefix "rescuex0011" 

.EXAMPLE
    Example for managed disk VM

    $scriptResult = .\New-AzRescueVM.ps1 -resourceGroupName recoveryVMRg -VmName recovmtestmg  -UserName "sujasd" -Password "XPa55w0rrd12345" -prefix "rescuex2" 

.EXAMPLE
    Example for marketplace image with Plan

    $scriptResult = .\New-AzRescueVM.ps1 -resourceGroupName recoverytest -VmName datasciencevm  -UserName "sujasd" -Password "XPa55w0rrd12345" -prefix "rescuex17" -AllowManagedVM

.EXAMPLE 
    Using a VM created from a custom image:

    $scriptResult =  .\New-AzRescueVM.ps1 -resourceGroupName testvmrecovery2 -VmName win2016custom  -UserName "sujasd" -Password "XPa55w0rrd12345" -prefix "rescuex18"

.EXAMPLE 
    Using a VM Unmanaged Windows VM

    $scriptResult =  .\New-AzRescueVM.ps1 -resourceGroupName testvmrecovery2 -VmName sujUNManagedvm  -UserName "sujasd" -Password "XPa55w0rrd12345" -prefix "rescuex48"

.NOTES
    Name: New-AzRescueVM.ps1

    To get help on the below script run get-help .\New-AzRescueVM.ps1
...
#Checks to see if Az is available
if (-not (get-module -ListAvailable -name 'Az.Accounts') -and (-not $env:ACC_CLOUD)) 
{
    $message = "[Error] Azure PowerShell not installed. Either install Azure PowerShell from https://docs.microsoft.com/en-us/powershell/azure/install-az-ps or use Cloud Shell PowerShell at https://shell.azure.com/powershell" 
...
else
{
    ...
    $authContext = Set-AzContext -SubscriptionId $subscriptionId
    write-log $authContext -logOnly
    if (-not $authContext.Subscription.Id)
    {
        $message = "[Error] Unable to set context to subscription ID $subscriptionId. Run Connect-AzAccount and then try the script again." 
        write-log $message -color red
        $scriptResult = Get-ScriptResultObject -scriptSucceeded $false -rescueScriptCommand $MyInvocation.Line -FailureReason $message -Scriptversion $scriptVersion -RunID $scriptRunId 
        return $scriptResult
    }
...
write-log "[Running] Get-AzVM -resourceGroupName $resourceGroupName -Name $vmName"
try
{
...
if ($managedVM)
{
    ...
    $oldDisk = Get-AzDisk -resourceGroupName $resourceGroupName -DiskName $OrignalosDiskName -WarningAction SilentlyContinue
    $location = $oldDisk.Location
    $diskConfig = New-AzDiskConfig -AccountType $storageType -Location $location -SourceResourceId $oldDisk.Id -CreateOption Copy -WarningAction SilentlyContinue
    $toBeFixedManagedOsDisk = Get-ValidLength -InputString $toBeFixedManagedOsDisk -Maxlength 80
    $disk = New-AzDisk -Disk $diskConfig -resourceGroupName $resourceGroupName -DiskName $toBeFixedManagedOsDisk -WarningAction SilentlyContinue
...
$started = Start-AzVM -resourceGroupName $rescueResourceGroupName -Name $rescuevm.Name
...
if ($windowsVM -and -not (RanFromCloudShell))
{
    ...
    Get-AzRemoteDesktopFile -resourceGroupName $rescueResourceGroupName -Name $rescuevm.Name -Launch 
}
...
if ($managedVM)
{
    $restoreScriptCommand = ".\Restore-AzOriginalVM.ps1 -resourceGroupName $resourceGroupName -VmName $vmName -subscriptionId $subscriptionId -diskName $diskname -prefix $prefix -OriginalProblemOSManagedDiskID $OriginalProblemOSManagedDiskID -OriginalDiskName $OrignalosDiskName" 
}
else
{
    $restoreScriptCommand = ".\Restore-AzOriginalVM.ps1 -resourceGroupName $resourceGroupName -VmName $vmName -subscriptionId $subscriptionId -FixedOsDiskUri $osDiskVHDToBeRepaired -OriginalosDiskVhdUri $OriginalosDiskVhdUri -prefix $prefix"
}
